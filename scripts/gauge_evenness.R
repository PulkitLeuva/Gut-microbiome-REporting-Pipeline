...
create_semi_circular_gauge <- function(
  ...
  color_breaks <- c("#FF6F6F", "#F6C357", "#81C784")
  ...
  p <- ggplot() +
    ...
    geom_text(data=scale_labels, aes(x=x, y=y, label=label), size=5, fontface="bold", color="#333333") +
    geom_text(aes(x=0, y=-0.1, label=paste0("Score= ", round(value, 2))), size=5, fontface="bold") +
    coord_fixed() +
    theme_void() +
...
evenness_value <- metrics$Evenness[1]
...
create_semi_circular_gauge(
  value = evenness_value,
  max_value = max_value,
  title = "Evenness",
  filename = output_file
)

# message(glue("Gauge plot saved at: {output_file}"))